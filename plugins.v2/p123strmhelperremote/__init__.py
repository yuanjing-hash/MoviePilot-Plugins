...
class P123StrmHelperRemote(_PluginBase):
    ...
    # 插件描述
    plugin_desc = "123云盘STRM生成一条龙服务，支持远程通知"
    # 插件图标
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "p123strmhelper_remote_"
    # 加载顺序
    plugin_order = 99
    # 可使用的用户级别
    auth_level = 1
    ...
    def get_api(self) -> List[Dict[str, Any]]:
        """
        BASE_URL: {server_url}/api/v1/plugin/P123StrmHelperRemote/redirect_url?apikey={APIKEY}
        0. 查询带 s3_key_flag
                                        ...
                                        "content": [
                                            {
                                                "component": "VSwitch",
                                                "props": {
                                                    "model": "enable_remote_notification",
                                                    "label": "启用远程通知接收",
                                                },
                                            }
                                        ],
    ...
    async def notify_upload_complete(self, request: Request, apikey: str = None):
        ...
        try:
            ...
            if not file_name or not pan_path:
                logger.error(f"【远程STRM通知】参数不完整: file_name={file_name}, pan_path={pan_path}")
                raise HTTPException(status_code=400, detail="file_name 和 pan_path 是必需的参数")
            
            logger.info(f"【远程STRM通知】接收到上传完成通知: {file_name}")
    ...
    async def _get_file_info_from_123(self, pan_path: str, file_name: str) -> dict:
        ...
        try:
            ...
            if not file_item:
                logger.error(f"【远程STRM通知】在123网盘中未找到文件: {full_path}")
                return None
    ...
    async def _generate_strm_from_notification(self, file_info: dict) -> dict:
        ...
        try:
            ...
            logger.error(f"【远程STRM生成】生成STRM时发生错误: {e}")
            return {
                "success": False,
                "message": f"STRM生成失败: {e}",
                "strm_path": "",
                "media_refresh": {"success": False, "message": "STRM生成失败，跳过媒体服务器刷新"},
                "library_info": {}
            }
    ...
    async def _create_strm_file(self, file_info: dict, strm_url: str) -> str:
        ...
        try:
            ...
            if not self._transfer_monitor_paths:
                logger.error("【远程STRM生成】未配置整理事件监控目录，无法生成STRM文件")
                return ""
            ...
            if not has_mapping:
                logger.error(f"【远程STRM生成】网盘路径 {pan_path} 未找到对应的本地路径映射")
                return ""
            ...
            try:
                ...
                logger.warning(f"【远程STRM生成】路径 {full_pan_path} 与基础目录 {pan_media_dir} 不匹配，将直接在 {local_media_dir} 下创建文件")
    ...
    async def _refresh_media_servers(self, file_info: dict, strm_path: str) -> dict:
        ...
        try:
            if not self.service_infos:
                logger.warning("【远程STRM生成】没有配置媒体服务器，跳过刷新")
                return {"success": False, "message": "没有配置媒体服务器"}
            ...
            for name, service in self.service_infos.items():
                try:
                    ...
                    if hasattr(service.instance, "refresh_root_library"):
                        ...
                        refresh_results[name] = {"success": False, "message": "不支持刷新"}
                        logger.warning(f"【远程STRM生成】媒体服务器 {name} 不支持刷新")
                except Exception as e:
                    refresh_results[name] = {"success": False, "message": str(e)}
                    logger.error(f"【远程STRM生成】媒体服务器 {name} 刷新失败: {e}")
            
            return {
                "success": any(result.get("success", False) for result in refresh_results.values()),
                "message": "媒体服务器刷新完成",
